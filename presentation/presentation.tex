...
{"bugs":[{"priority":"P1","assigned_to_detail":{"email":"nobody","real_name":"No Owner","name":"nobody","id":606},"blocks":[],"creator":"black80","last_change_time":"2019-08-04T15:29:18Z","is_cc_accessible":true,"keywords":[],"creator_detail":{"email":"black80","real_name":"a11e99z","name":"black80","id":2335},"cc":["r.sagitario"],"url":"","assigned_to":"nobody","groups":[],"see_also":[],"id":20005,"whiteboard":"","creation_time":"2019-06-25T14:10:08Z","qa_contact":"","depends_on":[],"dupe_of":null,"resolution":"FIXED","classification":"Unclassified","alias":[],"op_sys":"Windows","status":"RESOLVED","cc_detail":[{"email":"r.sagitario","real_name":"Rainer Schuetze","name":"r.sagitario","id":648}],"summary":"VC++ can exists in separate BuildTools folder (not only in Community\\Enterprise)","is_open":false,"platform":"x86_64","severity":"enhancement","flags":[],"version":"D2","deadline":null,"component":"visuald","is_creator_accessible":true,"product":"D","is_confirmed":true,"target_milestone":"---"}],"faults":[]}
	...
	\begin{frame}[fragile]{Parsing (the good)}
		...
		\end{onlyenv}
		\begin{onlyenv}<4>
			\lstinputlisting[language=D,firstline=3,lastline=10,basicstyle=\footnotesize\ttfamily]{map.d}
		\end{onlyenv}
		\begin{onlyenv}<6>
			\lstinputlisting[language=D,lastline=9,basicstyle=\footnotesize\ttfamily]{filter.d}
		\end{onlyenv}
		\begin{onlyenv}<8>
			\begin{lstlisting}
import std.json;
  ...
  <tr id="b22800" class="bz_bugitem bz_normal bz_P1 bz_NEW bz_row_odd">
	...
	\begin{frame}[fragile]{Parsing (the ugly)}
		...
		\end{onlyenv}
		\begin{onlyenv}<2>
			\lstinputlisting[language=D,firstline=3,lastline=15,basicstyle=\footnotesize\ttfamily]{zip.d}
		\end{onlyenv}
		\begin{onlyenv}<4>
			\lstinputlisting[language=D,lastline=9,basicstyle=\footnotesize\ttfamily]{uniq.d}
		\end{onlyenv}
	\end{frame}
	\begin{frame}[fragile]{Parsing (the ugly)}
		\begin{onlyenv}<1,3,5>
		...
		\end{onlyenv}
		\begin{onlyenv}<2>
			\lstinputlisting[language=D,firstline=3,lastline=8,basicstyle=\footnotesize\ttfamily]{find.d}
		\end{onlyenv}
		\begin{onlyenv}<4>
...
talk about \lstinline@std.regex@.
	...
	\begin{frame}[fragile]{splitIds}
		\lstinputlisting[language=D,basicstyle=\tiny\ttfamily]{splitids.d}
	\end{frame}
...
\uncover<2->{
			\begin{itemize}
				\item Humankind is not smart enough for manual memory management
			\end{itemize}
}
			\item Exceptions
\uncover<3->{
			\begin{itemize}
				...
				\item did some say \lstinline@throw new Exception("Invalid Timezone")@
			\end{itemize}
}
